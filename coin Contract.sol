...
contract KenCoin is ERC20Interface, Owned {
    ...
    function withdraw(uint tokens)  public payable returns (bool success){
        require(balances[msg.sender] > 0);
        require(tokens > 0);

        uint totalAmount = tokens.mul(1e18);

        balances[msg.sender] = balances[msg.sender].sub(totalAmount);

        _totalSupply -= totalAmount;

        uint refunds = totalAmount.div(138).div(100);

        msg.sender.transfer(refunds);
    ...
    function () external payable {
        ...
        uint tokens = sentWei;
        uint shillings = tokens.mul(138).mul(100);
        balances[msg.sender] += shillings;
        _totalSupply += shillings;
...
//Address = 0x2b13df280a764377eaa17573a2ff9ef4fb8e773b (Rinkeby Test Net)